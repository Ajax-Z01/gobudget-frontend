    ...
    <div className="hidden md:fixed md:inset-y-0 md:flex md:w-64 md:flex-col sidebar bg-[var(--background)] text-[var(--foreground)] border-r border-[var(--border-color)] dark:border-[var(--border-color)]">
      <div className="flex flex-col flex-grow pt-5 overflow-y-auto">
        ...
        <div className="flex items-center flex-shrink-0 px-4">
          ...
          />
          <span className="ml-2 text-xl font-semibold">GoBudget</span>
        </div>
        ...
        <div className="mt-5 flex-1 flex flex-col">
          <nav className="flex-1 px-2 pb-4 space-y-1">
            {[
              ...
              { name: "Budgets", icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" },
              { name: "Reports", icon: "M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" },
              { name: "Settings", icon: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" },
            ].map((item) => (
              <a
                key={item.name}
                href={`/${item.name.toLowerCase()}`}
                className="hover:bg-[var(--border-color)] dark:hover:bg-[var(--border-color)] group flex items-center px-2 py-2 text-sm font-medium rounded-md transition-colors duration-300"
              >
                <svg
